...
package net.mcreator.fight_extend_destroyed_civilization.world.dimension;
...
import net.mcreator.fight_extend_destroyed_civilization.procedures.MojingweiduDangWanJiaJinRuWeiDuShiProcedure;
import net.mcreator.fight_extend_destroyed_civilization.particle.PortalParticleParticle;
import net.mcreator.fight_extend_destroyed_civilization.item.MagicalCrystalWorldItem;
import net.mcreator.fight_extend_destroyed_civilization.block.PortalBlockBlock;
...
public class MagicalCrystalWorldDimension extends FightExtendDcModElements.ModElement {
	@ObjectHolder("fight_extend_dc:magical_crystal_world_portal")
	public static final CustomPortalBlock portal = null;

	public MagicalCrystalWorldDimension(FightExtendDcModElements instance) {
		super(instance, 20);
	...
	public void init(FMLCommonSetupEvent event) {
		...
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("fight_extend_dc:chaos_forest_biome")).getGenerationSettings()
				.getSurfaceBuilder().get().getConfig().getTop().getBlock());
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("fight_extend_dc:chaos_forest_biome")).getGenerationSettings()
				.getSurfaceBuilder().get().getConfig().getUnder().getBlock());
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("fight_extend_dc:magical_crystal_biome")).getGenerationSettings()
				.getSurfaceBuilder().get().getConfig().getTop().getBlock());
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("fight_extend_dc:magical_crystal_biome")).getGenerationSettings()
				.getSurfaceBuilder().get().getConfig().getUnder().getBlock());
	...
	public void clientLoad(FMLClientSetupEvent event) {
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				...
				effectsRegistry.put(new ResourceLocation("fight_extend_dc:magical_crystal_world"), customEffect);
			} catch (Exception e) {
				e.printStackTrace();
			}
		});
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("magical_crystal_world_portal", Vector3i::compareTo, 300);
	...
	public static class POIRegisterHandler {
		...
		public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
			poi = new PointOfInterestType("magical_crystal_world_portal",
					Sets.newHashSet(ImmutableSet.copyOf(portal.getStateContainer().getValidStates())), 0, 1)
							.setRegistryName("magical_crystal_world_portal");
	...
	public void initElements() {
		...
		elements.items.add(() -> new MagicalCrystalWorldItem().setRegistryName("magical_crystal_world"));
	}
	...
	public static class CustomPortalBlock extends NetherPortalBlock {
		...
		public void animateTick(BlockState state, World world, BlockPos pos, Random random) {
			for (int i = 0; i < 4; i++) {
				...
				world.addParticle(PortalParticleParticle.particle, px, py, pz, vx, vy, vz);
			}
			...
			if (!entity.isPassenger() && !entity.isBeingRidden() && entity.isNonBoss() && !entity.world.isRemote && true) {
				if (entity.func_242280_ah()) {
					entity.func_242279_ag();
						...
						new ResourceLocation("fight_extend_dc:magical_crystal_world"))) {
					entity.func_242279_ag();
					teleportToDimension(entity, pos,
							RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("fight_extend_dc:magical_crystal_world")));
				} else {
					entity.func_242279_ag();
	...
	public static class CustomPortalSize {
		private static final AbstractBlock.IPositionPredicate POSITION_PREDICATE = (state, blockReader, pos) -> {
			return state.getBlock() == PortalBlockBlock.block;
		};
				...
				for (int l1 = -1; l1 < 2; ++l1) {
					for (int k2 = 0; k2 < 2; ++k2) {
						for (int i3 = -1; i3 < 3; ++i3) {
							BlockState blockstate1 = i3 < 0 ? PortalBlockBlock.block.getDefaultState() : Blocks.AIR.getDefaultState();
			...
			for (int k1 = -1; k1 < 3; ++k1) {
				for (int i2 = -1; i2 < 4; ++i2) {
					if (k1 == -1 || k1 == 2 || i2 == -1 || i2 == 3) {
						...
						this.world.setBlockState(blockpos$mutable, PortalBlockBlock.block.getDefaultState(), 3);
					}
				}
			}
	...
	public void onPlayerChangedDimensionEvent(PlayerEvent.PlayerChangedDimensionEvent event) {
		...
		if (event.getTo() == RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("fight_extend_dc:magical_crystal_world"))) {

			MojingweiduDangWanJiaJinRuWeiDuShiProcedure.executeProcedure(Stream.of(new AbstractMap.SimpleEntry<>("entity", entity))
