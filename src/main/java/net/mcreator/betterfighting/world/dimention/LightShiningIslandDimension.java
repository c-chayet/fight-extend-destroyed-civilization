...
import net.mcreator.fight_extend_destroyed_civilization.particle.LightShiningIslandParticleParticle;
import net.mcreator.fight_extend_destroyed_civilization.item.LightShiningIslandItem;
import net.mcreator.fight_extend_destroyed_civilization.block.LightingChaosStoneBlock;
import net.mcreator.fight_extend_destroyed_civilization.block.LightStoneBlock;
...
public class LightShiningIslandDimension extends FightExtendDcModElements.ModElement {
	@ObjectHolder("fight_extend_dc:light_shining_island_portal")
	public static final CustomPortalBlock portal = null;

	public LightShiningIslandDimension(FightExtendDcModElements instance) {
		super(instance, 174);
	...
	public void init(FMLCommonSetupEvent event) {
		...
		replaceableBlocks.add(LightStoneBlock.block);
	...
	public void clientLoad(FMLClientSetupEvent event) {
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				...
				effectsRegistry.put(new ResourceLocation("fight_extend_dc:light_shining_island"), customEffect);
			} catch (Exception e) {
				e.printStackTrace();
			}
		});
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("light_shining_island_portal", Vector3i::compareTo, 300);
	...
	public static class POIRegisterHandler {
		...
		public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
			poi = new PointOfInterestType("light_shining_island_portal",
					Sets.newHashSet(ImmutableSet.copyOf(portal.getStateContainer().getValidStates())), 0, 1)
							.setRegistryName("light_shining_island_portal");
	...
	public void initElements() {
		...
		elements.items.add(() -> new LightShiningIslandItem().setRegistryName("light_shining_island"));
	}
	...
	public static class CustomPortalBlock extends NetherPortalBlock {
		...
		public void animateTick(BlockState state, World world, BlockPos pos, Random random) {
			for (int i = 0; i < 4; i++) {
				...
				world.addParticle(LightShiningIslandParticleParticle.particle, px, py, pz, vx, vy, vz);
			}
			...
			if (!entity.isPassenger() && !entity.isBeingRidden() && entity.isNonBoss() && !entity.world.isRemote && true) {
				if (entity.func_242280_ah()) {
					entity.func_242279_ag();
						...
						new ResourceLocation("fight_extend_dc:light_shining_island"))) {
					entity.func_242279_ag();
					teleportToDimension(entity, pos,
							RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("fight_extend_dc:light_shining_island")));
				} else {
					entity.func_242279_ag();
	...
	public static class CustomPortalSize {
		private static final AbstractBlock.IPositionPredicate POSITION_PREDICATE = (state, blockReader, pos) -> {
			return state.getBlock() == LightingChaosStoneBlock.block;
		};
				...
				for (int l1 = -1; l1 < 2; ++l1) {
					for (int k2 = 0; k2 < 2; ++k2) {
						for (int i3 = -1; i3 < 3; ++i3) {
							BlockState blockstate1 = i3 < 0 ? LightingChaosStoneBlock.block.getDefaultState() : Blocks.AIR.getDefaultState();
			...
			for (int k1 = -1; k1 < 3; ++k1) {
				for (int i2 = -1; i2 < 4; ++i2) {
					if (k1 == -1 || k1 == 2 || i2 == -1 || i2 == 3) {
						...
						this.world.setBlockState(blockpos$mutable, LightingChaosStoneBlock.block.getDefaultState(), 3);
					}
				}
			}
